...
Like the TI, APEDSK99 is based on good old through-hole technology. Besides the fact I don't like SMD soldering much, I also wanted to design something that fitted the TI era and could be put together by anybody with the same basic skills as myself. 
...
The DSR is based on the TI Disk Controller ROM, adapted to interface with a reliable SD card instead of wonky floppies. CRU is emulated through 2 memory mapped addresses, simplifying shield design. An 8KB binary DSR file is loaded into APEDSK99 RAM by the Arduino at powerup or reset. Various DSR optimisations have left about 3/4 KB RAM available for _TI BASIC_ support and future enhancements. 
