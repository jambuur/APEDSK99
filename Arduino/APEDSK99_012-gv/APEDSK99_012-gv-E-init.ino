void setup() {
  ...
  pinAsOutput(MY_HOLD);                                                 //Myarc HOLD; also en/disables the 74HC595 shift registers (and vice versa)
  MYstop();                                                             //halt Myarc and give Arduino RAM control 
  ...
  //... read sector/track with R6 counter in DSR RAM; see DSR source)
  pinAsInputPullUp(MY_INT);                                             //from Ethernet SS to detecting TI Interrupts 
  
  MYgo();                                                               //Myarc: take it away
