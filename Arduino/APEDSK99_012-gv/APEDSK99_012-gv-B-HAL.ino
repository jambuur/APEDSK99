...
//IO lines for Myarc control
#define MY_HOLD        3                                                      //PD0; manage Myarc HOLD* and enable/disable 74HC595 shift registers
#define MY_INT         2                                                      //PD2; GAL16V8 flag (DSR write & A15); shared with Ethernet CS (ETH_CS) 
#define MY_HOLDA      15                                                      //PC1; Myarc HOLDA* indication and 74HCT541 / GAL output buffers disable when INPUT
#define AD_BUFFERS    15                                                      //PC1; 74HCT541 / GAL output buffers enable (+ ERROR disable) when OUTPUT
...
//Short delay function to let Myarc finish the Instruction Acquisition (IAQ) cycle before HOLD is pulled LOW
...
//disable Myarc I/O, enable Arduino shift registers and control bus
//INLINE: need for speed
inline void MYstop() __attribute__((always_inline));
void MYstop( void ) {   
  IAQ();                                                                                                    //wait for Myarc to finish IAQ cycle
  digitalLow(MY_HOLD);                                                                                      //halts Myarc and enables 74HC595 shift registers
  while ( isLow(MY_HOLDA) ) {                                                                               //when HIGH disables 74HCT541 address buffers / GAL outputs
  }
  pinAsOutput(AD_BUFFERS);                                                                                  //aka MY_HOLDA
  digitalHigh(AD_BUFFERS);                                                                                  //when HIGH disables 74HCT541 address buffers / GAL outputs
  enable_controlbus();                                                                                      //Arduino in control of RAM
  NOP();                                                                                                    //wait for bus to settle
}

//enable Myarc I/O, disable Arduino shift registers and control bus
//INLINE: need for speed
inline void MYgo() __attribute__((always_inline));
void MYgo( void ) {
  disable_controlbus();                                                                                     //cease Arduino RAM control  
  NOP();    
  pinAsInput(MY_HOLDA);                                                                                                //wait for bus to settle                                                                
  digitalHigh(MY_HOLD);                                                                                     //disables 74HC595 shift registers and wakes up Myarc ...
                                                                                                            //... HOLDA* goes subsequently LOW, enabling 74HCT541 address buffers / GAL outputs                                                                    
} 
...
void Flasher( byte errorcode ) {                                                                      //error routine: stuck in code flashing loop until reset
  //"no APEDSK99 for you"
  pinAsOutput(AD_BUFFERS);                                                                            //disable ...         
  digitalHigh(AD_BUFFERS);                                                                            //... 74HCT541 address buffers / GAL outputs
  enable_controlbus();                                                                                //we have some flashing to do
  while ( true ) { 
