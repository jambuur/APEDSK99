    ...
    if ( FCcmd != FDINT ) { //do we need to do anything?
      ...
      if ( ACcmd != 0x00 ) {
        ...
        switch ( ACcmd ) {
          ...
          {
            if ( Dbtidx == 0 ) {                                              //first run of FDSK command?
              cDSK = Rbyte(DTCDSK);                                           //yes; get requested DSKx ...
              if ( aDSK[cDSK] ) {                                             //DOAD mapped to DSKx?
                Dbtidx = 256;                                                 //yes; set first FDR pointer
            ...
            if ( Sbtidx != 0 ) {                                              //valid FDR pointer?
              DSK[cDSK].seek(Sbtidx * 256);                                   //yes; locate FDR     
